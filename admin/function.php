 ...
 */ 
   function pagination($query, $per_page = 2,$p = 1){    
        ...
        if($_GET['page'] == "servers" && $_GET['g_name']){
		  $q = addslashes($_GET['g_name']);
		  $vtid = "/admin/?page=servers&g_name=$q";
		  ...
		  $vtid = "/admin/?page=servers&hostname=$hostname&ip=$ip&game=$game&location=$location&mod=$mod";
		} else if($_GET['page'] == "servers"){
		    if($_GET['addedby']){
            $id = addslashes($_GET['addedby']);
			$vtid = "/admin/?page=servers&addedby=$id";				  
		    } else if($_GET['ownedby']){
			$id = addslashes($_GET['ownedby']);
			$vtid = "/admin/?page=servers&ownedby=$id";				
			} else {
		    $vtid = "/admin/?page=servers";
			}
		} else if($_GET['page'] == "servers" && $_GET['addedby']){
			$id = addslashes($_GET['addedby']);
			$vtid = "/admin/?page=servers&addedby=$id";	
		} else if($_GET['page'] == "servers" && $_GET['ownedby']){
			$id = addslashes($_GET['ownedby']);
			$vtid = "/admin/?page=servers&ownedby=$id";	
			...
			$vtid = "/admin/?page=memberlist&name=$name";
			} else {
			$vtid = "/admin/?page=memberlist";	
			}
			...
			if($_GET['name']){ 
			$name = addslashes($_GET['name']);
			$vtid = "/admin/?page=communities&name=$name";
			} else {
			$vtid = "/admin/?page=communities";	
			}
			
		}  else if($_GET['page'] == "messages"){
		  $vtid = "/admin/?page=messages";
		} else if($_GET['page'] == "user_log"){
		  $vtid = "/admin/?page=user_log";
		} else if($_GET['page'] == "admin_log"){
		  $vtid = "/admin/?page=admin_log";
		}
    	...
    	{	
    		$pagination .= "<ul class='pagination'>";
                    $pagination .= "<li class='details'>Page $p of $lastpage</li>";
