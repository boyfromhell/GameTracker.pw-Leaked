...
foreach($results as $data){
	...
	if($data['gq_online'] == "1"){
		...
		$update_query		= mysql_query("UPDATE servers SET num_players='$num_players', max_players='$max_players', playercount='$server_playercount', chart_updates='$chart_updates', last_chart_update='$timestamp', rank_pts='$rank_pts' WHERE id='$server_id'");
	} else {
		if($server_game == "cs16" OR $server_game == "css" OR $server_game == "csgo" OR $server_game == "cod2" OR $server_game == "cod4" OR $server_game == "tf2"){
		$server_playercount		= substr($server_playercount,3);
		...
		$update_query			= mysql_query("UPDATE servers SET online='0', playercount='$server_playercount', last_chart_update='$timestamp' WHERE id='$server_id'");
	}
